...
if __name__ == "__main__":
    ...
    # ##
    normenv = VecNormalize(vecenv, norm_obs=False, norm_reward=True, clip_reward=10.0, gamma = env.gamma)
    obs = normenv.reset()
    print('ENV made')
    ...
    model = PPO("CnnPolicy", normenv, gamma=env.gamma, verbose=0, 
                    ...
                    batch_size=256, 
                    learning_rate=get_linear_fn(4e-4,1e-5,0.5), 
                    # n_epochs=4,
                    tensorboard_log="./ppo_logs/",
                    # ent_coef=0.01,
                    clip_range = 0.15,
    ...
    # model = PPO.load("/home/ckang/projects/corona2025/local_resnet18policy_len365_risk0.03_ib-4.0_cb0.04_ci0.1_it5000000_2.zip", 
    #                  env=normenv, device="cuda",
    #                  custom_objects={
    #                         "learning_rate": 1e-5,            # 하이퍼파라미터 값 교체
    #                         "lr_schedule": lambda _: 1e-5,    # 스케줄도 상수로 덮어쓰기
    #                     },
    #                  )
    
    # %%
    for _ in tqdm(range(env.epis_length*2)):  # 2만~5만 스텝 권장
    ...
    log_name = prefix+"_len"+str(env.epis_length)+"_risk"+str(env.risk_coexist)+ \
        "_ib"+str(init_bias)+ \
